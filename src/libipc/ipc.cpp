      ...
      it = storages
               .emplace(chunk_size,
                        chunk_handle_ptr_t{
                            ...
                            [](chunk_handle_t *p) { ipc::mem::destruct(p); }})
               .first;
    }
  }
...
struct detail_impl {
  ...
  static bool wait_for_recv(ipc::handle_t h, std::size_t r_count,
    ...
    return wait_for(
        ...
        [que, r_count] { return que->conn_count() < r_count; }, tm);
  }
  ...
  static bool send(F &&gen_push, ipc::handle_t h, void const *data,
    ...
    if (data == nullptr || size == 0) {
      if (verbose) ipc::error("fail: send(%p, %zd)\n", data, size);
      return false;
    }
    auto que = queue_of(h);
    if (que == nullptr) {
      if (verbose) ipc::error("fail: send, queue_of(h) == nullptr\n");
      return false;
    }
    if (que->elems() == nullptr) {
      if (verbose) ipc::error("fail: send, queue_of(h)->elems() == nullptr\n");
      return false;
    }
    if (!que->ready_sending()) {
      if (verbose) ipc::error("fail: send, que->ready_sending() == false\n");
      return false;
    }
    ipc::circ::cc_t conns =
    ...
    auto acc = inf->acc();
    if (acc == nullptr) {
      if (verbose) ipc::error("fail: send, info_of(h)->acc() == nullptr\n");
      return false;
    }
        ...
        [tm](auto *info, auto *que, auto msg_id) {
          return [tm, info, que, msg_id](std::int32_t remain, void const *data,
            ...
            if (!wait_for(
                    info->wt_waiter_,
                    [&] {
                      return !que->push([](void *) { return true; },
  ...
  static ipc::buff_t recv(ipc::handle_t h, std::uint64_t tm, bool verbose) {
    ...
    for (;;) {
      ...
      std::int32_t r_size =
          static_cast<std::int32_t>(ipc::data_length) + msg.remain_;
      if (r_size <= 0) {
        if (verbose) ipc::error("fail: recv, r_size = %d\n", (int)r_size);
        return {};
      }
            ...
            return ipc::buff_t{
                ...
                [](void *p_info, std::size_t size) {
                  ...
                  IPC_UNUSED_ auto finally =
                      ipc::guard([r_info] { ipc::mem::free(r_info); });
