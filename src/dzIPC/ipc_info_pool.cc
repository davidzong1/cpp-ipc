...
namespace dzIPC
{
    namespace info_pool
    {
        namespace
        {
            ...
            constexpr uint32_t kMagic = 0x5A495044u; // 'DZIP'
            ...
            static_assert(ATOMIC_INT_LOCK_FREE == 2,
                          "std::atomic<uint32_t> must be lock-free for cross-process sharing");
            static_assert(ATOMIC_LLONG_LOCK_FREE == 2,
                          "std::atomic<int64_t> must be lock-free for cross-process sharing");
            static_assert(std::is_trivially_copyable<PoolEntry>::value ||
                              std::is_standard_layout<PoolEntry>::value,
                          "PoolEntry must be standard layout for shm mapping");

            constexpr std::size_t kRegionSize =
                sizeof(PoolHeader) + sizeof(PoolEntry) * kMaxEntries;
            ...
            struct ScopedShmLock
            {
                ...
                bool locked{false};
                explicit ScopedShmLock(pthread_mutex_t *mtx) : m(mtx)
                {
        ...
        struct IpcInfoPool::Impl
        {
            ...
            Impl()
            {
                if (!shm.acquire(kShmName, kRegionSize,
                ...
                if (header->init_state.compare_exchange_strong(
                        expect, kInitInProgress, std::memory_order_acq_rel))
                {
                    header->magic = kMagic;
        ...
        IpcInfoPool::IpcInfoPool() : impl_(new Impl()) {}
