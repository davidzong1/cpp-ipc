...
namespace
{
    ...
    std::string format_age(int64_t register_ts_ns)
    {
        ...
        int64_t now_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(
                             ...
                             .count();
        int64_t age_s = (now_ns - register_ts_ns) / 1000000000LL;
        if (age_s < 0)
            age_s = 0;
        char buf[32];
        if (age_s < 60)
            ...
            std::snprintf(buf, sizeof(buf), "%lldh%02lldm", static_cast<long long>(age_s / 3600),
    ...
    void print_snapshot(const std::vector<dzIPC::info_pool::EntrySnapshot> &entries, const Options &opt)
    {
        ...
        std::sort(view.begin(), view.end(), [](const auto *a, const auto *b)
                  {
                      ...
                      if (a->topic_name != b->topic_name) return a->topic_name < b->topic_name;
                      return a->pid < b->pid; });
        ...
        for (const auto *e : view)
        {
            std::cout << std::left
                      << std::setw(4) << e->slot
                      << std::setw(15) << dzIPC::info_pool::to_string(e->kind)
                      << std::setw(8) << e->pid
                      << std::setw(7) << e->domain_id
                      << std::setw(8) << format_age(e->register_ts_ns)
                      << std::setw(32) << e->topic_name
                      << (e->type_name.empty() ? "-" : e->type_name);
            if (!e->alive)
                std::cout << "  [DEAD]";
            std::cout << "\n";
            if (opt.verbose && !e->extra.empty())
            {
                std::cout << "     └── " << e->extra << "\n";
            }
        }
        std::cout << "\nTotal: " << view.size() << " entr" << (view.size() == 1 ? "y" : "ies") << std::endl;
    }
