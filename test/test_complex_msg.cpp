...
public:
    ...
    // 生成随机测试数据
    ComplexMessage generateRandomMessage()
    {
        ComplexMessage msg;
        ...
        std::uniform_int_distribution<int16_t> int16_dis(-32'768, 32'767);
        std::uniform_int_distribution<uint16_t> uint16_dis(0, 65'535);
        std::uniform_int_distribution<int32_t> int32_dis(-2'147'483'648, 2'147'483'647);
        std::uniform_int_distribution<uint32_t> uint32_dis(0, 4'294'967'295U);
        std::uniform_int_distribution<int64_t> int64_dis(-9'223'372'036'854'775'807LL, 9'223'372'036'854'775'807LL);
        std::uniform_int_distribution<uint64_t> uint64_dis(0, 18'446'744'073'709'551'615ULL);
        ...
        for (int i = 0; i < array_size; ++i)
        {
            msg.message_array.push_back("String " + std::to_string(i) + "_" + std::to_string(gen() % 1'000));
        }
    ...
    // 验证两个消息是否相等
    bool verifyMessages(const ComplexMessage& original, const ComplexMessage& deserialized)
    {
        ...
        if (original.big_uint != deserialized.big_uint)
            return false;
        if (std::abs(original.single_precision - deserialized.single_precision) > 1e-6f)
            return false;
        if (std::abs(original.double_precision - deserialized.double_precision) > 1e-12)
            return false;
        ...
        // 浮点数组需要特殊处理
        if (original.single_precision_array.size() != deserialized.single_precision_array.size())
            return false;
        for (size_t i = 0; i < original.single_precision_array.size(); ++i)
        {
            if (std::abs(original.single_precision_array[i] - deserialized.single_precision_array[i]) > 1e-6f)
                return false;
        }

        if (original.double_precision_array.size() != deserialized.double_precision_array.size())
            return false;
        for (size_t i = 0; i < original.double_precision_array.size(); ++i)
        {
            if (std::abs(original.double_precision_array[i] - deserialized.double_precision_array[i]) > 1e-12)
                return false;
        }
    ...
    // 性能测试
    void performanceTest(int iterations = 1'000)
    {
        std::cout << "\n=== 性能测试 ===" << std::endl;
        ...
        std::cout << "序列化耗时: " << serialize_time.count() << " 微秒" << std::endl;
        std::cout << "平均每次序列化: " << serialize_time.count() / iterations << " 微秒" << std::endl;
        std::cout << "序列化后大小: " << serialized_data.size() << " 字节" << std::endl;

        // 反序列化性能测试
        start = std::chrono::high_resolution_clock::now();
        ComplexMessage deserialized;
        for (int i = 0; i < iterations; ++i)
        {
        ...
        auto deserialize_time = std::chrono::duration_cast<std::chrono::microseconds>(end - start);
        std::cout << "反序列化耗时: " << deserialize_time.count() << " 微秒" << std::endl;
        std::cout << "平均每次反序列化: " << deserialize_time.count() / iterations << " 微秒" << std::endl;
    ...
    void runAllTests()
    {
        ...
        for (int i = 0; i < total; ++i)
        {
            ...
            ComplexMessage original = generateRandomMessage();
            ...
            if ((i + 1) % 20 == 0)
            {
                std::cout << "完成 " << i + 1 << "/" << total << " 个测试..." << std::endl;
            }
        }
